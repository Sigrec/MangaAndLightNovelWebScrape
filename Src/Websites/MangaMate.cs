...
public partial class MangaMate
{
    ...
    public const string WEBSITE_URL = "https://mangamate.shop";
    ...
    internal async Task CreateMangaMateTask(string bookTitle, BookType bookType, ConcurrentBag<List<EntryModel>> MasterDataList, WebDriver driver)
    {
        await Task.Run(() => 
        {
    ...
    private string GenerateWebsiteUrl(string bookTitle, BookType bookType, ushort pageNum)
    {
        ...
        string url = $"{WEBSITE_URL}/search?options%5Bprefix%5D=last&page={pageNum}&q={InternalHelpers.FilterBookTitle(bookTitle.Replace(" ", "+"))}+{(bookType == BookType.Manga ? "manga" : "novel")}";
