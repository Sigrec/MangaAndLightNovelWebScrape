...
namespace MangaAndLightNovelWebScrape.Websites
{
    public partial class BarnesAndNoble
    {
        ...
        private static readonly XPathExpression TitleXPath = XPathExpression.Compile("//div[contains(@class, 'product-shelf-title product-info-title pt-xs')]/a");
        private static readonly XPathExpression OneShotTitleXPath = XPathExpression.Compile("//div[@id='commerce-zone']//h1[@itemprop='name']");
        private static readonly XPathExpression PriceXPath = XPathExpression.Compile("//div[@class='product-shelf-pricing mt-xs']//div//a//span[2]");
        private static readonly XPathExpression OneShotPriceXPath = XPathExpression.Compile("(//span[@id='pdp-cur-price'])[1]");
        private static readonly XPathExpression StockStatusXPath = XPathExpression.Compile("//p[@class='ml-xxs bopis-badge-message mt-0 mb-0' and (contains(text(), 'Online') or contains(text(), 'Pre-order'))]");
        ...
        [GeneratedRegex(@"(?<=Vol \d{1,3})[^\d{1,3}.]+.*|,|(?<=Omnibus\s\d{1,3})[^\d.].*| \([^()]*\)|The Manga|Manga")]  private static partial Regex ParseTitleRegex();
        ...
        protected internal async Task CreateBarnesAndNobleTask(string bookTitle, BookType book, bool isMember, List<List<EntryModel>> MasterDataList)
        {
            await Task.Run(() => 
            {
                MasterDataList.Add(GetBarnesAndNobleData(bookTitle, book, isMember));
            });
        }
        ...
        private string GenerateWebsiteUrl(BookType bookType, string bookTitle, bool check)
        {
        ...
        private static string TitleParse(string entryTitle, BookType bookType, string bookTitle, bool oneShotCheck)
        {
            ...
            else
            {
                ...
                LOGGER.Debug("(1) {}", entryTitle);
                
                if (entryTitle.Contains("(B&N Exclusive Edition)"))
                {
                    entryTitle = entryTitle.Insert(entryTitle.AsSpan().IndexOf("Vol"), "B&N Exclusive Edition ");
                }
                LOGGER.Debug("(2) {}", entryTitle);
        ...
        private List<EntryModel> GetBarnesAndNobleData(string bookTitle, BookType bookType, bool memberStatus)
        {
            try
            {
                bool hardcoverCheck = false, secondCheck = false, oneShotCheck = false;
                string originalUrl = GenerateWebsiteUrl(bookType, bookTitle, secondCheck);
                HtmlDocument doc = new HtmlDocument();
                HtmlWeb web = new HtmlWeb { UserAgent = string.Empty, UsingCacheIfExists = true };

                CheckOther:
                List<KeyValuePair<string, string>> ValidUrls = new List<KeyValuePair<string, string>>();
                int validUrlCount = 0;
                ...
                else
                {
                    ...
                    if (formats.Count != 0)
                    {
                        ...
                        foreach(HtmlNode format in formats)
                        {
                            ...
                            {
                                ValidUrls.Add(new KeyValuePair<string, string>(new string($"{UrlFixRegex().Replace(url, string.Empty)}/?Nrpp=40&Ns=P_Display_Name%7C0&page=1"), "Hardcover"));
                            }
                            else if (!hardcoverCheck && innerText.Equals("BN Exclusive", StringComparison.OrdinalIgnoreCase))
                            {
                                ValidUrls.Add(new KeyValuePair<string, string>(new string(url), "OneShot"));
                            }
                            else
                            {
                                ValidUrls.Add(new KeyValuePair<string, string>(new string($"{UrlFixRegex().Replace(url, string.Empty)}/?Nrpp=40&Ns=P_Display_Name%7C0&page=1"), "Paperback"));
                            }
                        }
                    }
                }
                ...
                while (oneShotCheck || (validUrlCount <= ValidUrls.Count))
                {
                    ...
                    else if (oneShotCheck) validUrlCount++;
                    ...
                    if (!doc.ParsedText.Contains("The page you requested can't be found") && !doc.ParsedText.Contains("Sorry, we couldn't find what you're looking for"))
                    {
                        ...
                        if (!oneShotCheck && doc.DocumentNode.SelectSingleNode("//div[@id='productDetail-container']") != null) oneShotCheck = true;
                    }
                    else goto Quit;
                        ...
                        if (!oneShotCheck
                            ...
                            || (
                                    ...
                                    && (
                                            ...
                                            || InternalHelpers.RemoveUnintendedVolumes(bookTitle, "One Piece", entryTitle, "Formation of the Spade Pirates", "Vol. 2: New World")
                    ...
                    if (pageCheck != null)
                    {
                        nextPage = doc.DocumentNode.SelectSingleNode("//a[@class='next-button']").GetAttributeValue("href", "No Url");
                    ...
                    else if (oneShotCheck || validUrlCount == ValidUrls.Count) break;
                }
            }
            catch (Exception e)
            {
