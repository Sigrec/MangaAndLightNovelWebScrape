...
namespace Tests
{
    ...
    public class ParsingTests
    {
        ...
        {
            Assert.Multiple(() => {
               Assert.That(BarnesAndNoble.TitleParse("Classroom of the Elite: Horikita (Manga) Vol. 1", 'M', "classroom of the elite"), Is.EqualTo("Classroom of the Elite Horikita Vol 1"));
               Assert.That(BarnesAndNoble.TitleParse("One Piece, Vol. 97", 'M', "one piece"), Is.EqualTo("One Piece Vol 97")); 
               Assert.That(BarnesAndNoble.TitleParse("One Piece (Omnibus Edition), Vol. 1: East Blue Vols. 1-2-3", 'M', "one piece"), Is.EqualTo("One Piece Omnibus Vol 1")); 
               Assert.That(BarnesAndNoble.TitleParse("One Piece Box Set 4: Dressrosa to Reverie: Volumes 71-90 with Premium", 'M', "one piece"), Is.EqualTo("One Piece Box Set 4"));
